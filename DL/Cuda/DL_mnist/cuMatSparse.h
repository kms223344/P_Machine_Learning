...
public:
    ...
    */
    ...
    void s_s_dot(cuMatSparse &b, cuMatSparse &c){
        ...
        size_t num;
        cusparseStatus_t status = cusparseScsrgemm2(cuHandle,
                rows,
                b.cols,
                cols, &alpha,
                descr,
                ...
                c.csrValDevice,
                c.csrRowPtrDevice,
                c.csrColIndDevice,info,&num);
        void* buffer = NULL;
        cudaMalloc(&buffer, num);
        cusparseStatus_t status =
                cusparseScsrgemm2(cuHandle,
                                 rows,
                                 b.cols,
                                 cols,&alpha,
                                 descr,
    ...
    void transpose(cuMatSparse &r){
        void* buffer;
        size_t num;
        cusparseStatus_t status = cusparseCsr2cscEx2_bufferSize(cuHandle, rows, cols, numVals,   //cusparseCsr2cscEx2
                                      csrValDevice, csrRowPtrDevice,
                                      csrColIndDevice, r.csrValDevice,
                                      r.csrColIndDevice, r.csrRowPtrDevice, CUDA_R_32F,
                                      CUSPARSE_ACTION_NUMERIC,
                                      CUSPARSE_INDEX_BASE_ZERO, CUSPARSE_CSR2CSC_ALG1,&num);
        if (status != CUSPARSE_STATUS_SUCCESS) {
            cout << "transpose error" << endl;
        }
        cudaMalloc(&buffer, num);
        cusparseStatus_t status = cusparseCsr2cscEx2(cuHandle, rows, cols, numVals,   //cusparseCsr2cscEx2
                         csrValDevice, csrRowPtrDevice,
                         csrColIndDevice, r.csrValDevice,
                         r.csrColIndDevice, r.csrRowPtrDevice, CUDA_R_32F,
                         CUSPARSE_ACTION_NUMERIC,
